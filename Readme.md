...
>- 一个函数值是否可以赋值给一个函数类型，首先要看函数值的参数类型、参数顺序、返回值类型是否与函数类型声明的形式兼容一致，其次要看函数值声明的Exceptions(Throws子句)能否被函数类型声明的Exceptions(Throws子句)覆盖包含，立即函数-Lambda没有Throws子句，因此不用考虑Exception的兼容性
