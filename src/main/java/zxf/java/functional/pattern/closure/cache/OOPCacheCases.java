 ...
 * 闭包：Caching.cachedFunction 把缓存封装在返回的 Function 闭包内部，外部无法直接访问状态；
 * OOP：OOPCache 用实例字段 cache 维护状态，状态对外可见、需显式管理。
...
public class OOPCacheCases {
    ...
    public static void use_case1() {
        ...
        // OOP 方式：缓存是对象的可见状态字段（OOPCache.cache）
        ...
        System.out.println("--- 结论：闭包把状态隐藏在函数内，OOP 把状态暴露为字段 ---");
    }
}
